...
#Graphical Printout
bargraph(~toJapan | gender, success = "Yes", data = dataH1, type = "percent", main = "Percentage Male and Female applying for Japan", xlab = "Applying for Japan", ylab = "Percentage")
cat("------------------------ Percentage students going to Japan (by gender)", fill=TRUE)
prop(requestedCountry~gender, success = "Japan", data = dataH1)

#Analyze Data
cat("------------------------ Entries for bootstrap calculation (by gender)", fill=TRUE)
female <- dataH1 %>% filter(gender == "Female")
cat(nrow(female), fill=TRUE, labels = "Using n entries for calculation of female entries: ")
male <- dataH1 %>% filter(gender == "Male")
cat(nrow(male), fill=TRUE, labels = "Using n entries for calculation of male entries: ")

propMaleToJapan <- prop(~toJapan, success = "Yes", data = male)
cat(propMaleToJapan, fill=TRUE, labels = "Percentage male going to Japan (Value to compare in bootstrap method): ")
...
histogram(~ bootPercentFemale$t, xlab = "Percent Female", ylab = "Density", main="Percent Female going to Japan")
...
#Graphical printout
xyplot(y ~ x, data=dataH2, xlab="Customers' Age [Years]", ylab="Time span from oportunity to deal [Day(s)]", main="Time span from oportunity to deal [Day(s)] ~ Customers' Age [Years]")
