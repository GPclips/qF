...
prepareDataH3 <- function(){
	...
	dataH3 <- merge(rename(bipDeutschlandBis2017, c("Jahr"="year", "bip_mrd_euro"="billion_eur")),rename(students_amount_y10_GER, c("amount"="amountStudents")),by=c("year")) %>% removeMissings()
	...
	cat(nrow(dataH3), fill=TRUE, labels = "Using n entries for calculation: ")
	return(dataH3)
}
